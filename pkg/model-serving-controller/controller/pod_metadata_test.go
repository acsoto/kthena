...
func TestCreatePodRestoresReservedMetadataAfterTemplateAndPlugin(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = utils.CreateControllerRevision(ctx, kubeClient, ms, "stale-revision", []workloadv1alpha1.Role{role})
	require.NoError(t, err)
	require.NoError(t, utils.CleanupOldControllerRevisions(ctx, kubeClient, ms))
	_, err = kubeClient.AppsV1().ControllerRevisions(ms.Namespace).Get(ctx, utils.GenerateControllerRevisionName(ms.Name, revision), metav1.GetOptions{})
	require.NoError(t, err, "retention must discover the authoritative Pod revision")
	_, err = kubeClient.AppsV1().ControllerRevisions(ms.Namespace).Get(ctx, utils.GenerateControllerRevisionName(ms.Name, "stale-revision"), metav1.GetOptions{})
	assert.True(t, apierrors.IsNotFound(err), "unused revision should be removed")
}
